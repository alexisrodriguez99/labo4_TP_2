...
@Component({
  selector: 'app-reporte-turnos-medico',
...
export class ReporteTurnosMedicoComponent implements OnInit {
  ...
  ngOnInit(): void {
console.log(this.users.listadoEspecialistas)
    ...
    this.initSubscribe = this.turnos.traerTodos().subscribe(turnos => {
      turnos.forEach(turno => {
        this.turnosEspecialistas.forEach(element => {
          if(turno.idEspecialista == element.especialista.id && turno.estado!='libre')
          {
            element.turnos++;
          }
        });
  ...
  Filtrar()
  {
    ...
    {
      this.initSubscribe = this.turnos.traerTodosReportes().subscribe(turnos => {
        this.turnosEspecialistas.forEach(element => {
          element.turnos=0
        })
        turnos.forEach(turno => {
